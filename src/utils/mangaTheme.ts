...
// Warm terracotta + sepia + gold "manga panel" aesthetic.
export const MANGA = {
  ink: '#1A0F0A',
  paper: '#FFF7EC',
  panel: '#E8D9C0',
...
export const MANGA_STATUS: Record<
  MangaEstado,
...
// Hard offset "comic panel" shadow (no blur). Faithful on iOS via shadowRadius 0;
// Android falls back to a soft elevation shadow.
export function hardShadow(
  dx = 4,
  dy = 4,
  color: string = MANGA.ink
): ViewStyle {
  return {
    shadowColor: color,
