...
export const useMangasStore = create<MangasState>((set, get) => ({
  ...
  addManga: async (input) => {
    const manga = await db.insertManga(input);
    set((s) => ({ mangas: [...s.mangas, manga] }));
    return manga;
  },

  updateManga: async (id, patch) => {
    const updated = await db.updateManga(id, patch);
    if (updated) set((s) => ({ mangas: s.mangas.map((m) => m.id === id ? updated : m) }));
  },

  deleteManga: async (id) => {
    await db.deleteManga(id);
    set((s) => ({ mangas: s.mangas.filter((m) => m.id !== id) }));
  },
  ...
  advanceManga: async (id, delta = 1) => {
    ...
    set((s) => ({ mangas: s.mangas.map((m) => m.id === id ? { ...m, leidos, estado } : m) }));
    try {
      await db.updateManga(id, { leidos, estado });
    } catch {
